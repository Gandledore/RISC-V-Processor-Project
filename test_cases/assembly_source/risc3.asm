...
loop:
        beq  t1, t2, done        # if index == 3, exit loop
        ...
        addi t6, zero, 100       # target address = 100
...
process:
        andi t6, t4, 1           # check LSB: even if (val & 1 == 0)
        beq  t6, zero, is_even   # branch if even

        # odd path:
        add  t5, t4, zero       # return value unchanged
        jalr zero, ra, 0

is_even:
        andi t5, t4, 0xF0        # mask high bits
